...
| Weapon         | Properties                                                                      |
| -------------- | ------------------------------------------------------------------------------- |
| Carbine        | +1 ATK when Engaged in Tight battlefields. −1 ATK when Engaged in Open battlefields. |
| Assault Rifle  | No special properties. The reliable workhorse.                                  |
| Marksman Rifle | +1 ATK when Engaged in Exposed battlefields. −1 ATK when Engaged in Dense battlefields. |
...
Each Trooper has a base [[Mobility]] of 5. Armor and gear reduce this value. Mobility is the target number for all [[Mobility Check|Mobility Checks]]: roll 1d6 equal or under to succeed. A Trooper with Mobility 0 automatically fails all Mobility Checks. Being wounded imposes −1 Mobility.
...
### Utility Kit — `REQ: 0` — Mobility: No cost

Smokes, flares, flashbangs. The essentials. *Active (1 Ammo each):*
...
### LMG — `REQ: 1` — Mobility: −1

Light automatic weapon for mobile suppression and supporting fire.

*Passive:* An additional +1 DEF for a Trooper receiving Covering Fire.

### Plasma Rifle — `REQ: 3` — Mobility: −1

A devastating energy weapon prone to catastrophic feedback.

*Active:* Roll {roll:1d6|Plasma} on use. Does not require Ammo.
...
### Rocket Launcher — `REQ: 1` — Mobility: −1

Anti-armor launcher designed to destroy vehicles and strongpoints.

*Active:* +3 ATK — or deal direct damage to a [[Hard Target]], counting as 2 Hits. Trooper is in Exposed position after use. **Single use.**

### Melee Weapon — `REQ: 0` — Mobility: −1

A large maul, chainsaw sword or similar device. Not available in every setting.

*Passive:* When a Trooper Moves Up to a Flanking position, they can choose to move to a Flanked position instead of rolling. Doing so grants +3 ATK. This already includes a bonus for Flanking.

### Grenade Launcher — `REQ: 1` — Mobility: −1

Indirect-fire weapon used to flush enemies or hit hard targets.

*Active:* Launch a grenade against a Hard Target (counts as a Hit), or at another Trooper's target, granting them the benefit of Flanking in the next Offense Roll. Multiple grenades can be fired in one attack — each costing 1 Ammo.

### Sniper Rifle — `REQ: 1` — Mobility: −1

Long-range rifle optimized for precision shots from cover.

*Passive:* +1 ATK when Fortified. An additional +1 ATK when Fortified and didn't Move in the previous Exchange.

### HMG — `REQ: 2` — Mobility: −2

Heavy-caliber weapon best deployed from a braced or fortified position.

*Active:* Provide Covering Fire for up to 3 Troopers this round. Costs 1 Ammo regardless of number of Troopers covered.

*Passive:* +1 ATK when Fortified.

## Special Equipment

A Trooper can carry **1 piece of Special Equipment**.

### Commander's Kit — `REQ: 0` — Mobility: No cost

Encrypted comms headset, wrist-mounted tactical interface, signal override module — all optimized for quick squad coordination.

Whenever Enemy Tactics are triggered, roll {roll:1d6|Commander}. On a **1–3**, the Tactic is nullified as the Commander adapts the squad's response in time.

### Drone Gear — `REQ: 0` — Mobility: −1

Backpack with a deployable aerial recon drone.

Add +1 to each Advance Roll. Does not stack with multiple Drone Gear.

### Jump Pack — `REQ: 2` — Mobility: −1
...
### Supply Backpack — `REQ: 1` — Mobility: −1
...
### Demolition Charges — `REQ: 0` — Mobility: −1
